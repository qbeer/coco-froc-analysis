...
from froc.bootstrap_curve import 

def run(args):
    if args.bootstrap:
        print('Generating bootstrap curves... (this may take a while)')
        generate_bootstrap_curves(args.gt_ann, args.pred_ann, args.n_bootstrap_samples, 
                                  args.use_iou, args.iou_thres, args.plot_title, args.plot_output_path)
    else:
        print('Generating single FROC curve...')
        generate_froc_curve(args.gt_ann, args.pred_ann, args.use_iou, args.iou_thres, args.plot_title, args.plot_output_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--bootstrap', action="store_true", default=False, help='Whether to do a single or bootstrap runs.')
